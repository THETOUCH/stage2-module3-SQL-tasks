select * from Payment
                  AS p JOIN PaymentType PT on PT.id = p.type_id where PT.name = 'MONTHLY';
...
SELECT s2.id, s2.name, s2.birthday, s2.groupnumber FROM Mark AS m JOIN Subject S on S.id = m.subject_id JOIN Student S2 on S2.id = m.student_id WHERE S.name = 'Math';